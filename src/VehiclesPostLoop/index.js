...
wp.blocks.registerBlockType("green-blocks/vehicles-post-loop", {
  ...
  attributes: {
    ...
    button: {
      type: "string",
      default:
        "bg-slate-700 !text-slate-50 hover:!bg-slate-50 hover:!text-slate-700 hover:!border-slate-50",
    },
  },
  edit: function (props) {
            ...
            data.map(async (vehicle) => {
              try {
                const res = await fetch(
                  vehicle._links["wp:featuredmedia"][0].href
                );
                const data = await res.json();
                ...
                return { ...vehicle, featured_image: null };
              }
            })
          );
          ...
          <SelectControl
            ...
            options={[
              ...
              },
              {
                label: "Light Button",
                value:
                  "bg-slate-50 !text-slate-700 hover:bg-slate-700 hover:!text-slate-50 hover:border-slate-700",
              },
            ]}
            ...
            {vehicles &&
              vehicles.map((vehicle) => (
                <div key={vehicle.id} className="border rounded-md p-5">
                  ...
                  <p class="text-sm  mb-2 w-fit py-1 px-2 bg-slate-500 text-slate-50 rounded-md duration-200 ease-in-out transition hover:bg-slate-700">
                    Vehicle Type
                  </p>
      ...
      <>
        ...
        >
          <div className={`${props.attributes.column_width}`}>
            <div className={`w-full grid ${props.attributes.grid_width}`}></div>
          </div>
        </div>
      </>
    );
